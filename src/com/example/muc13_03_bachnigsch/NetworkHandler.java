...
public class NetworkHandler {
	...
	private class DownloadWebpageTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... urls) {
			...
			try {
				// fetches and processes the web page content. When it finishes,
		...
		protected void onPostExecute(String result) {
			Log.d(DEBUG_TAG, "Empfangener String: " + result);
			...
			if (sendMode == false) {
				try {
					...
					// Alle Elemente aus Empfangenem String auswerten und nach
					// Benutzer "mussi0815" suchen
					for (int i = 0; i < list.length(); i++) {
						JSONObject listObject = list.optJSONObject(i);
						// get username from preferences
						SharedPreferences sharedPref = PreferenceManager
								.getDefaultSharedPreferences(searchActivity);
						String buddysname = sharedPref.getString(
								"pref_buddysname", "");
						
						Log.i(DEBUG_TAG, "Buddy's name is "+buddysname);

						if (listObject.getString("user").equals(buddysname)) {
				...
				Log.d(DEBUG_TAG, "Received Azimut: " + receivedAzimut);
	...
	// a string.
	private String downloadUrl(String myurl, String requestMethod)
			throws IOException {
